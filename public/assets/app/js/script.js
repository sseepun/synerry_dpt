$(function(){ 'use strict';
    ...
    // Banner 02
    if($('section.banner-02').length){
        $('section.banner-02 > .slide-container').each(function(){
            var self = $(this);
            self.find('> .slides').slick({
                centerMode: true, centerPadding: 0, slidesToShow: 1, 
                focusOnSelect: true, autoplay: true, autoplaySpeed: 4000, speed: 800,
    ...
    // Content 03
    if($('.content-03').length){
        $('.content-03').each(function(){
            var self = $(this),
            selectors = self.find('.selector');
